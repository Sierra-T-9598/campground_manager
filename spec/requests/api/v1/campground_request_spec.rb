...
RSpec.describe 'Campground Endpoints' do
  ...
  describe 'patch /campgrounds/:id' do
    describe 'request to update a campground with existing id' do
      it 'generates an updated campground response' do
        ...
        previous_name = campground_1.name
        expect(previous_name).to eq('Moraine Park')
        ...
        expect(updated_campground.booked_dates).to be_an Array
  ...
  describe 'get /campgrounds?from={date}&to={date}' do
    describe 'request to get all campgrounds with availability between given dates' do
      it 'returns a json list of available campgrounds' do
        from = '5/6/22'
        to = '5/8/22'
        get "/api/v1/campgrounds?from=#{from}&to=#{to}"

        campgrounds = JSON.parse(response.body, symbolize_names: true)
        expect(response.status).to eq(200)

        expect(campgrounds).to have_key(:data)
        expect(campgrounds[:data].count).to eq(2)
  ...
  describe 'request to order campgrounds by price' do
    describe 'get /campgrounds?order=low_to_high' do
      it 'returns list of campgrounds ordered by lowest average price first' do
        get "/api/v1/campgrounds?order=low_to_high"
        ...
        expect(campgrounds).to have_key(:data)
        expect(campgrounds[:data].count).to eq(3)
        expect(campgrounds[:data].first[:attributes][:name]).to eq(campground_2.name)
        expect(campgrounds[:data].last[:attributes][:name]).to eq(campground_1.name)

      end
    end

    describe 'get /campgrounds?order=high_to_low' do
      it 'returns list of campgrounds ordered by highest average price first' do
        get "/api/v1/campgrounds?order=high_to_low"
        ...
        expect(campgrounds).to have_key(:data)
        expect(campgrounds[:data].count).to eq(3)
        expect(campgrounds[:data].first[:attributes][:name]).to eq(campground_1.name)
        expect(campgrounds[:data].last[:attributes][:name]).to eq(campground_2.name)
      end
    end
  end
end
